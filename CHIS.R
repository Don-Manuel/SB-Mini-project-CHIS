...
adult <- adult[adult$BMI_P <= 52 & adult$BMI_P > 16, ]
...
ggplot(DF_melted, aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax, fill = FILL)) +
...
ggplot(DF_all, aes(ymin = ymin, ymax = ymax, 
xmin = xmin, 
xmax = xmax,
 fill = residual)) +
  geom_rect() +
...
geom_text(aes(x = max(xmax), y = ytext,
label = FILL), 
size = 3,
...
g <- ggplot(DF_all, aes(ymin = ymin,  ymax = ymax, xmin = xmin, xmax = xmax, fill = residual)) +
  geom_rect(col = "white") +
  geom_text(aes(x = xtext, label = X),y = 1, size = 3, angle = 90, hjust = 1, show.legend = FALSE) +
